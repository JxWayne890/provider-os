    ...
    return (
        <div className="space-y-10 animate-reveal pb-20">
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-5xl font-serif font-bold text-[#1D1D1F] tracking-tight">Operations Hub</h2>
                    <p className="text-[#86868B] mt-2 font-medium tracking-wide uppercase text-xs">Internal task matrix and automated system protocols</p>
                </div>
            ...
            <div className="luminous-card bg-white overflow-hidden">
                <div className="p-6 border-b border-[#F5F5F7] bg-[#F5F5F7]/30 flex justify-between items-center">
                    <div className="flex gap-8">
                        <button className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#B8860B] border-b-2 border-[#B8860B] pb-1">All Missions</button>
                        <button className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#86868B] hover:text-[#1D1D1F] transition-colors pb-1">By Projection</button>
                        <button className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#86868B] hover:text-[#1D1D1F] transition-colors pb-1">My Deck</button>
                    </div>
                    <button className="text-[#86868B] hover:text-[#1D1D1F] transition-all p-2 hover:bg-white rounded-xl">
                        <Filter size={18} />
                    </button>
                </div>
                ...
                <div className="divide-y divide-[#F5F5F7]">
                    {tasks.map((task) => (
                        <div key={task.id} className="p-6 hover:bg-[#F5F5F7]/30 transition-all flex items-center gap-6 group">
                            <button className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${task.status === 'Done' ? 'bg-[#1D9D60] border-[#1D9D60]' : 'border-[#E8E8E8] hover:border-[#B8860B]'}`}>
                                {task.status === 'Done' && <CheckCircle2 size={14} className="text-white" />}
                            </button>

                            <div className="flex-1">
                                <p className={`text-sm font-medium ${task.status === 'Done' ? 'text-[#86868B] line-through' : 'text-[#1D1D1F]'}`}>
                                    {task.description}
                                </p>
                                <div className="flex items-center gap-6 mt-3">
                                    <div className="flex items-center gap-2">
                                        {task.priority === 'High' && <AlertTriangle size={12} className="text-red-500" />}
                                        {task.priority === 'Medium' && <Briefcase size={12} className="text-[#B8860B]" />}
                                        {task.priority === 'Low' && <CheckCircle2 size={12} className="text-[#1D9D60]" />}
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-[#86868B]">{task.priority} Priority</span>
                                    </div>
                            ...
                            <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                {task.notes && (
                                    <div className="px-2 py-1 bg-[#F5F5F7] border border-black/5 rounded-lg text-[9px] text-[#B8860B] font-bold uppercase tracking-widest">Protocol Attached</div>
                                )}
                                <button className="p-2.5 text-[#86868B] hover:text-[#1D1D1F] hover:bg-white rounded-xl transition-all shadow-sm">
